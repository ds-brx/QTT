...
# TorchVision: PyTorch's Computer Vision library [Computer software]. 
...
from torchvision.transforms import (
    functional as F, InterpolationMode, Compose, Resize, ToTensor,Normalize
)
...
def get_dataset(is_train, args):
    ...
    id2label = json.load(open(hf_hub_download(
        args.dataset_name, "id2label.json", repo_type="dataset"), "r"))
    num_classes = len(id2label)

    dataset = handle_data.SemanticSegTorchDataset(dataset, get_transform(is_train, args))
...
def main(args):
    ...
    if args.use_v2 and args.dataset_name != "coco":
    ...
    dataset, num_classes = get_dataset(True, args)
    dataset_test, _ = get_dataset(False, args)
    ...
    data_loader_test = torch.utils.data.DataLoader(
        dataset_test, batch_size=1, sampler=test_sampler, num_workers=args.dataloader_num_workers, collate_fn=utils.collate_fn
    )
    ...
    if args.aux_loss:
        ...
        params_to_optimize.append({"params": params, "lr": args.learning_rate * 10})
    optimizer = torch.optim.SGD(params_to_optimize, lr=args.learning_rate, momentum=args.momentum, weight_decay=args.weight_decay)

    scaler = torch.cuda.amp.GradScaler() if args.amp else None

    iters_per_epoch = len(data_loader)
    main_lr_scheduler = PolynomialLR(
        optimizer, total_iters=iters_per_epoch * (args.num_train_epochs - args.warmup_steps), power=0.9
    )

    if args.warmup_steps > 0:
        warmup_iters = iters_per_epoch * args.warmup_steps
        args.lr_scheduler_type = args.lr_scheduler_type.lower()
        if args.lr_scheduler_type == "linear":
            ...
            raise RuntimeError(
                f"Invalid warmup lr method '{args.lr_scheduler_type}'. Only linear and constant are supported."
            )
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument("--model_name_or_path", default="fcn_resnet101", type=str, help="model name")
    ...
    parser.add_argument("--warmup_steps", default=0, type=int, help="the number of epochs to warmup (default: 0)")
    parser.add_argument("--lr_scheduler_type", default="linear", type=str, help="the warmup method (default: linear)")
